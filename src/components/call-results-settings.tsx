...
import {
  createCallResultOptionAction,
  removeCallResultOptionAction,
  saveAllCallResultOptionsAction,
  setCallResultActiveAction,
  type CallResultOptionSaveRow,
} from "@/app/actions";
...
export type CallResultOptionRow = {
  code: string;
  label: string;
  isBuiltIn: boolean;
  active: boolean;
  /** Null when unset in DB — resolved per outcome code for display and picker default. */
...
type CallResultsSettingsProps = {
  options: CallResultOptionRow[];
};

function buildDraftsFromOptions(opts: CallResultOptionRow[]): Record<string, RowDraft> {
  return Object.fromEntries(
    opts.map((o) => [
      o.code,
      {
        label: o.label,
        accentKey: resolveCallResultAccentKey(o.accentKey, o.code),
    ...
    <tr className="border-b border-slate-100 align-middle last:border-b-0">
      ...
      <td className="px-4 py-4 sm:px-5 md:w-[260px]">
        ...
        <CallResultAccentPicker
          key={pickerKey}
          defaultAccentKey={resolveCallResultAccentKey(opt.accentKey, opt.code)}
      ...
      <td className="px-4 py-4 sm:px-5">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <form action={setCallResultActiveAction} className="inline">
            ...
            <button
              ...
              title={
                opt.active
                  ? "Hide this result when logging new calls"
                  : "Show this result again for new calls"
              }
              className={cn(
          ...
          {opt.isBuiltIn ? (
            <button
              type="button"
              disabled
              title="Built-in statuses can’t be deleted — hide them from new calls instead."
            ...
            <form action={removeCallResultOptionAction} className="inline">
              <input type="hidden" name="code" value={opt.code} />
              <button
                type="submit"
...
export function CallResultsSettings({ options }: CallResultsSettingsProps) {
  ...
  const saveAll = useCallback(() => {
    setSaveError(null);
    const rows: CallResultOptionSaveRow[] = options.map((o) => {
    ...
    startTransition(() => {
      void saveAllCallResultOptionsAction(rows)
        .then((result) => {
          if (result?.ok === false) {
  ...
  return (
    <div className="space-y-8">
      <p className="text-sm text-slate-600">
        Edit labels and colors below, then use <span className="font-medium text-slate-800">Save all changes</span> once to
        update the database. Hide/show and Delete still run immediately.
      </p>

      <section className="rounded-[22px] border border-slate-200/90 bg-slate-50/40 p-4 sm:p-5">
        <h4 className="text-sm font-semibold text-slate-800">Add custom result</h4>
        <p className="mt-1 text-xs text-slate-500">Creates a new status as soon as you submit this row.</p>
        <form
          action={createCallResultOptionAction}
          className="mt-4 grid gap-4 sm:grid-cols-[minmax(0,1fr)_minmax(200px,240px)_auto] sm:items-end"
        >
          <div className="min-w-0">
            <label htmlFor="new-call-result" className="mb-1 block text-xs font-medium text-slate-600">
              Label
            </label>
            <input
              id="new-call-result"
      ...
      <section className="overflow-hidden rounded-[22px] border border-slate-200/90 bg-white shadow-sm">
        <div className="border-b border-slate-100 bg-slate-50/80 px-4 py-3 sm:px-5">
          <h4 className="text-sm font-semibold text-slate-800">All call result statuses</h4>
          <p className="mt-0.5 text-xs text-slate-500">Same columns for every row — scroll horizontally on small screens.</p>
        </div>
