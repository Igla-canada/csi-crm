...
-- Backfill: rows with a recording are done.
UPDATE "CallLog"
SET
...
-- No RingCentral link — cron ignores (null).
UPDATE "CallLog"
SET "telephonyRecordingEnrichStatus" = NULL, "telephonyRecordingEnrichNextAt" = NULL
WHERE "ringCentralCallLogId" IS NULL;

-- Still missing recording: only recent rows get automatic retries (older left null unless touched by app).
UPDATE "CallLog"
SET
